...
class PostNLApi {
  ...
  async archiveLetters(liveLetters, archivedLetters) {
    ...
    const cutoffKey = this._localDateKey(cutoffDate);
    return [...merged.values()]
      .filter(item => {
        const key = this._localDateKey(item.deliveryDate || item.archivedAt);
        return key && key >= cutoffKey;
      })
      .sort((a, b) => new Date(b.deliveryDate || b.archivedAt) - new Date(a.deliveryDate || a.archivedAt))
      .slice(0, 60);
  }

...
class PostNLApi {
  ...
  async request(url, options = {}, retry = true) {
    ...
    if (response.status === 401 && retry && this.auth?.refreshToken) {
      // Another parallel request may already have refreshed the token. Reuse it first;
      // only force a refresh when this request actually used the current token.
  ...
  async fetchPackages() {
    ...
    return shipments.map(item => ({
      ...
      delivered: Bolean(item.delivered),
